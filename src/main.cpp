...
#if defined(NDEBUG)
# error "EarthDollar cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd4846f68bf72354de32c4d1cf6ea3f550090cb56c564572f4f0d4b419dd02592");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EarthDollar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EarthDollar Signed Message:\n";
...
{
    // EarthDollar: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
  ...
  if (nHeight < 2)
{nSubsidy = 1800000000 * COIN;}

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 2 * 60; // EarthDollar: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // EarthDollar: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EarthDollar: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= 4) { DiffMode = 2; }
	}
...
void ThreadScriptCheck() {
    RenameThread("EarthDollar-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EarthDollar: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8629f24c75d9983493df4fda38a8aa1b00d54ca3ba668c4e39c775563de88e8c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x175b03001bb3bc9e6842ab0954f2dbc5142b528966f4a981039e27436d3a75a3"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // EarthDollar: increase each by adding 2 to EarthDollar's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EarthDollarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EarthDollarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EarthDollarMiner(CWallet *pwallet)
{
    printf("EarthDollarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EarthDollar-miner");
    ...
    try { loop {
        ...
        printf("Running EarthDollarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EarthDollarMiner terminated\n");
        throw;
    }
}

void GenerateEarthDollars(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EarthDollarMiner, pwallet));
}
